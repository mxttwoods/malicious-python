...
getmac_output = (
    subprocess.run("getmac", capture_output=True).stdout.decode().split("\n")
)
...
controller_key_part = (
    r"SYSTEM\ControlSet001\Control\Class\{4d36e972-e325-11ce-bfc1-08002be10318}"
)
...
with winreg.ConnectRegistry(None, winreg.HKEY_LOCAL_MACHINE) as hkey:
    ...
    controller_key_folders = [
        ("\\000" + str(item) if item < 10 else "\\00" + str(item))
    ...
    for key_folder in controller_key_folders:
        ...
        try:
            ...
            with winreg.OpenKey(
                hkey, controller_key_part + key_folder, 0, winreg.KEY_ALL_ACCESS
            ) as regkey:
                ...
                try:
                    ...
                    while True:
                        ...
                        if (
                            ...
                            and value == mac_addresses[int(option)][1]
                        ):
...
while run_last_part:
    ...
    network_adapters = (
        subprocess.run(["wmic", "nic", "get", "name,index"], capture_output=True)
    ...
    mac_add = "-".join(
        [
            (mac_to_change_to[int(update_option)][i : i + 2])
